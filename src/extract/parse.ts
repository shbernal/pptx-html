...
export function parseDeckHtml(fullHtml) {
...
export function composeSlideDocument(headHTML, slideHTML, lang) {
	const exportCss = '<style>body.magic-exporting .slide,body.magic-exporting .frag,body.exporting .slide,body.exporting .frag{transition:none!important;animation:none!important;opacity:1!important;visibility:visible!important;transform:none!important}</style>'
	return '<!doctype html><html lang="' + escapeAttr(lang || 'en') + '"><head><meta charset="utf-8">' +
		headHTML + exportCss + '</head><body class="exporting magic-exporting">' + slideHTML + '</body></html>'
}
